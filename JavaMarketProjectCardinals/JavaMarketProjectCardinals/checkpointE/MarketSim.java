...
public class MarketSim {
	...
	public static void main(String[] args) {
		...
		while (test) {
			// pull data from sql to find last runID
			int id = MarketDB.getRunIDFromDB();
			...
			// update runID on full service lanes
			lanes.setRunID(id + 1);
			...
			// update runID to self checkouts
			selfServeLanes.setRunID(lanes.getRunID());
				...
				if (clockTime != 0) {
					...
					if (custGenerationNum <= custAmt) {
						...
						if (clockTime == arrivalTimes[custGenerationNum - 1]) {
							...
							newCust.setRunID(lanes.getRunID());
			...
			s.nextLine();
			String save = s.nextLine().toLowerCase();

			boolean valid = false;
			while (!valid) {
				if (!save.equals("no") && !save.equals("yes")) {
					System.err.println("That is not a valid option.");
					System.out.println("\nWould you like to save this run to the database? (yes or no)");
					save = s.nextLine().toLowerCase();
				} else {
					valid = true;
					if (save.equals("yes")) {
	...
	public static void customerSatisfaction(ArrayList<Customer> custs, int cFull, int cSelf) {
		...
		// if number of unsatisfied customers is greater than 20% of customers & lanes
